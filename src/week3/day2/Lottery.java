...
/*"Лотерея"
    ...
    Программа вывод в консоль выпавшые номера в порядке возрастания(использовать один из методов в Вашем классе ArrayUtils).
...
//        int[] gamerNumbers = enterNumbers(1, 42);
    ...
    private static void showCongrats(int guessedNumbers) {
        switch (guessedNumbers){
            ...
            case 6:
                ...
                break;
            default:
                System.out.printf("Try more! You guess %d numbers", guessedNumbers);
        }
    }
    ...
    private static int[] generateNumbers(int min, int max) {
        final int size = 6; // <- "final" - 
        int[] lotteryNumbers = new int[size];

        for (int i = 0; i < size; i++) {
            int randomNumber = (int)(Math.random() * (max + 1 - min) + min); // <- "max + 1" -
